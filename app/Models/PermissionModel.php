...
class PermissionModel extends Model
{
    ...
    {
        $query = DB::table('pref_permission_names')
            ->join('pref_permission', 'pref_permission_names.permission_id', '=', 'pref_permission.id')
            ->where([
                ['pref_permission_names.lang', '=', $lang],
                ['pref_permission.status', '!=', config('constants.STATUS_DELETE')],
            ])
            ->select(
                'pref_permission.id',
        ...
        if ($term) {
            $query->where('pref_permission_names.name', 'like', "%{$term}%");
        }
