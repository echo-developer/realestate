...
@section('content')
    ...
    <div class="app-main__inner">
        ...
        @endif
        @if ($errors->has('xlsFileHospital'))
            <div class="alert alert-danger alert-dismissible mt-2">
                {{ $errors->first('xlsFileHospital') }}
                <button type="button" class="btn-close" data-bs-dismiss="alert">
    ...
    <div class="modal fade" id="excel_upload_modal" tabindex="-1" role="dialog" aria-labelledby="excel_upload_modal"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="excel_upload_form" action="{{ route('hospital.import-excel') }}" method="POST"
                    enctype="multipart/form-data">
                    <div class="modal-body">
                        @csrf
                        <input type="file" class="form-control" name="xlsFileHospital">
                        ...
                        <div class="mt-3 p-3 bg-light border border-danger rounded">
                            ...
                            <a href="{{ asset(config('constants.HOSPITAL_EXCEL_FORMAT')) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            function saveData() {
                ...
                $.ajax({
                    url: id ? `{{ route('hospital.update', ':id') }}`.replace(':id', id) :
                        `{{ route('hospital.store') }}`,
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
