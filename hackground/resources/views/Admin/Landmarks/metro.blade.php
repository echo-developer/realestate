...
<div class="app-main__inner">
        ...
        <div class="page-title-wrapper">
            ...
            <div class="page-title-actions">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">Metro Station List</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="main-card mb-3 card">
        <div class="card-header d-flex">
            <h4>Metro Station List</h4>

            <div class="btn-actions-pane-right">
                <div class="btn-group" id="all-select" style="display: none;">
                    <button type="button" class="btn btn-default btn-sm" data-toggle="tooltip" title="" id="delete-all" data-original-title="Delete selected"><i class="fa fa-trash"></i></button>
                    <button type="button" class="btn btn-default btn-sm" data-toggle="tooltip" title="" id="activate-all" data-original-title="Make active"><i class="fa-regular fa-thumbs-up"></i>
                        <button type="button" class="btn btn-default btn-sm" data-toggle="tooltip" title="" id="deactivate-all" data-original-title="Make inactive"><i class="fa-solid fa-thumbs-down"></i></button>
                </div>
                ...
                <table class="mb-0 table">
                    ...
                    <tbody id="user">
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <a href="javascript:void(0)" class="me-2 edit" data-id="{{$data->id}}"><i class="bi bi-pencil-square text-success fa-md "></i></a>
                                <a href="javascript:void(0)" class="delete" data-id="{{$data->id}}"><i class="bi bi-trash3-fill text-danger fa-md"></i></a>
                            </td>
                        </tr>
                        @empty
                        <tr>
...
<script>
    $(document).ready(function() {
        ...
        function saveData() {
            ...
            $.ajax({
                url: id ? `{{ route('metro.update', ':id') }}`.replace(':id', id) : `{{ route('metro.store') }}`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function editData() {
            ...
            let id = $(this).data('id');
            $.ajax({
                url: "{{ route('metro.edit', ':id') }}".replace(':id', id),
                type: 'GET',
                success: function(response) {
        ...
        function statusUpdate() {
            ...
            $.ajax({
                url: `{{route('metro.status')}}`.replace(':id', id),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    status: status,
        ...
        function deleteData() {
            ...
            if (confirm('Are you sure you want to delete this item?')) {
                $.ajax({
                    url: `{{route('metro.destroy', ':id')}}`.replace(':id', id),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
        ...
        function deleteAll() {
            ...
            if (selectedIds.length > 0) {
                if (confirm('Are you sure you want to delete all selected items?')) {
                    $.ajax({
                        url: `{{route('metro.delete-multiple')}}`,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: selectedIds
        ...
        function activateAll() {
            ...
            if (selectedIds.length > 0) {
                if (confirm('Are you sure you want to activate all selected items?')) {
                    $.ajax({
                        url: `{{route('metro.activate-multiple')}}`,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: selectedIds
        ...
        function deactivateAll() {
            ...
            if (selectedIds.length > 0) {
                if (confirm('Are you sure you want to deactivate all selected items?')) { // Confirmation dialog
                    $.ajax({
                        url: `{{route('metro.deactivate-multiple')}}`, // Your backend route
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: selectedIds
