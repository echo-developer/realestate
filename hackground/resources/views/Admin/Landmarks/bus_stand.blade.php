        ...
        <div class="page-title-wrapper">
            ...
            <div class="page-title-actions">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">Bus Stand List</li>
                </ol>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        function saveData() {
            ...
            $.ajax({
                url: id ? `{{ route('bus.update', ':id') }}`.replace(':id', id) : `{{ route('bus.store') }}`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function editData() {
            ...
            let id = $(this).data('id');
            $.ajax({
                url: "{{ route('bus.edit', ':id') }}".replace(':id', id),
                type: 'GET',
                success: function(response) {
        ...
        function statusUpdate() {
            ...
            $.ajax({
                url: `{{route('bus.status')}}`.replace(':id', id),
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    status: status,
        ...
        function deleteData() {
            ...
            if (confirm('Are you sure you want to delete this item?')) {
                $.ajax({
                    url: `{{route('bus.destroy', ':id')}}`.replace(':id', id),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
        ...
        function activateAll() {
            ...
            if (selectedIds.length > 0) {
                if (confirm('Are you sure you want to activate all selected items?')) {
                    $.ajax({
                        url: `{{route('bus.activate-multiple')}}`,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: selectedIds
        ...
        function deactivateAll() {
            ...
            if (selectedIds.length > 0) {
                if (confirm('Are you sure you want to deactivate all selected items?')) { // Confirmation dialog
                    $.ajax({
                        url: `{{route('bus.deactivate-multiple')}}`, // Your backend route
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: selectedIds
