...
<div class="app-main__inner">
        ...
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>Notification Template
                    <div class="page-title-subheading">Management &gt; Notification Template List</div>
                </div>
            </div>
            <div class="page-title-actions">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ url('/') }}"> Home</a></li>
                    <li class="breadcrumb-item active">Notification Template List</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <style>
        ...
        @media (max-width: 767px) {
            ...
            #prop_NotificationTemplate .modal-dialog { position: fixed; bottom: 0; left: 0; right: 0; margin: 0; width: 100%; max-width: 100%; transform: translateY(100%); transition: transform 0.35s cubic-bezier(0.32, 0.72, 0, 1); }
            #prop_NotificationTemplate.show .modal-dialog { transform: translateY(0); }
            #prop_NotificationTemplate .modal-content { border-radius: 20px 20px 0 0; border: none; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 -4px 24px rgba(0,0,0,0.15); }
            #prop_NotificationTemplate .modal-header { border-bottom: 1px solid #f0f0f0; padding: 0.85rem 1.25rem 0.75rem; }
            #prop_NotificationTemplate .modal-body { overflow-y: auto; -webkit-overflow-scrolling: touch; padding: 1rem 1.25rem; }
            #prop_NotificationTemplate .modal-footer { border-top: 1px solid #f0f0f0; padding: 0.75rem 1.25rem; padding-bottom: calc(0.75rem + env(safe-area-inset-bottom)); }
            #prop_NotificationTemplate .form-floating .form-control, #prop_NotificationTemplate .form-floating .form-select { height: 52px; font-size: 0.95rem; }
            #prop_NotificationTemplateButton { width: 100%; height: 46px; border-radius: 12px; font-weight: 600; }
        }
    ...
    <form action="{{ url('management/notificationtemplate') }}" method="get">
        <section class="content-header mb-2">
            <div class="row justify-content-end">
                <div class="col-xl-4 col-lg-6">
                    <div class="input-group">
                        <input class="form-control" id="prop_NotificationTemplate_search" placeholder="Search..."
    ...
    <div class="main-card mb-3 card card-modern">
        <div class="settings-card-header">
            <h4><i class="fa fa-bell text-primary me-2"></i> Notification Template List</h4>
            <div class="btn-actions-pane-right">
                <button type="button" id="exportExcel" class="btn btn-sm btn-outline-primary mr-2" style="border-radius: 8px;">
                    <i class="fa fa-download"></i> Export CSV
                </button>
                <button type="button" class="btn-add-setting" onclick="add_prop_NotificationTemplate()">
        ...
        <div class="card-body">
            <div class="table-responsive" id="main_table">
                <table class="settings-table" id="Notification-table">
                    <thead>
                        <tr>
                    ...
                    <tbody id="user">
                        ...
                        <tr>
                            ...
                            <td data-label="Template for"><span>{{ $item->template_for }}</span></td>
                            <td data-label="Template Key"><span>{{ $item->template_key }}</span></td>
                            <td data-label="Order"><span>{{ $item->order }}</span></td>
                            <td data-label="Status">
                                <div>
                                    <input type="checkbox" class="NotificationTemplate_prop_status d-none"
                            ...
                            <td data-label="Action" class="text-right">
                                <div class="actions-cell">
                                    <a href="javascript:void(0)" class="action-icon-btn edit cursor-pointer"
                                        onclick="Edit_prop_NotificationTemplate('{{ $item->id }}')">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <a href="javascript:void(0)" class="action-icon-btn delete cursor-pointer"
                                        onclick="Delete_prop_NotificationTemplate('{{ $item->id }}')">
...
@endsection
@section('modals')
<div class="modal fade" id="prop_NotificationTemplate" tabindex="-1" role="dialog"
    aria-labelledby="prop_NotificationTemplateaddEditModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header border-bottom-0">

                <h5 class="modal-title fw-bold" id="prop_NotificationTemplateAddEditModalLabel"></h5>
            ...
            <div class="modal-body">
                ...
                <form id="prop_NotificationTemplateformData">
                    <input type="hidden" class='d-none' id="prop_NotificationTemplateimage" name="image">
                    <input type="text" class='d-none' id="prop_NotificationTemplateId" name="prop_NotificationTemplateId">
                    @php
            ...
            <div class="modal-footer border-top-0">
                <button type="button" onclick="add_edit_prop_NotificationTemplate()" id="prop_NotificationTemplateButton"
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#exportExcel').on('click', function() {
            ...
            var table = document.querySelector('#Notification-table').cloneNode(true);


            $(table).find('input.NotificationTemplate_prop_status').each(function() {
            ...
            XLSX.writeFile(workbook, 'Notification-template.xlsx');
        });
    });
    ...
    function add_prop_NotificationTemplate() {
        ...
        prop_NotificationTemplateAddEdit('Add Notification Template', 'Add');
    }

    function Edit_prop_NotificationTemplate(id) {
        console.log(id);
        $('.form-control').removeClass('is-invalid');
        $('.invalid-feedback').empty();
        prop_NotificationTemplateAddEdit('Edit Notification Template', 'Update', id);
    }

    function prop_NotificationTemplateAddEdit(title, buttonText, id = null) {
        $('#prop_NotificationTemplateAddEditModalLabel').text(title);
        $('#prop_NotificationTemplateButton').text(buttonText);
        $('#prop_NotificationTemplateformData')[0].reset();
        $('#image_preview').attr('src', '').hide();
        $('#delete_image_btn').hide();
        if (id) {
            $.get(`{{ url('/management/notificationtemplate/details') }}/${id}`, function(data) {
                $('#prop_NotificationTemplateId').val(data[0].notification_templates_id);
                data.forEach(function(NotificationTemplate) {

                    const editorInstance = CKEDITOR.instances['content_' + NotificationTemplate.lang];
                    if (editorInstance) {
                        editorInstance.setData(NotificationTemplate
                            .content); // Use CKEditor's method to set data
                    } else {
                        $('#content_' + NotificationTemplate.lang).val(NotificationTemplate
                            .content); // Fallback in case CKEditor is not initialized
                    }


                    if (NotificationTemplate.lang === 'en') {
                        $('#name').val(NotificationTemplate.template_for);
                        $('#template_key').val(NotificationTemplate.template_key).prop('readonly', true);
                        $('#all_template_keys').val(NotificationTemplate.all_template_keys).prop('readonly', true);
                        $('#order').val(NotificationTemplate.order);
                        $('input[name="status"][value="' + NotificationTemplate.status + '"]').prop(
                            'checked', true);
                    }
                });
            });
        }
        $('#prop_NotificationTemplate').modal('show');
    }

    function add_edit_prop_NotificationTemplate() {

        for (let instance in CKEDITOR.instances) {
            CKEDITOR.instances[instance].updateElement(); // This updates the textarea with CKEditor content
        }

        var data = $("#prop_NotificationTemplateformData").serializeArray();
        $.ajaxSetup({
            headers: {
        ...
        var url = $('#prop_NotificationTemplateId').val() ?
            `{{ url('/management/notificationtemplate/edit') }}` :
            `{{ url('/management/notificationtemplate/add') }}`;
    ...
    $('.NotificationTemplate_prop_status').change(function() {
        ...
        $.ajax({
            type: 'POST',
            url: `{{ url('management/notificationtemplate/status') }}`,
            data: {
                'status': status,
                'id': id
            },
    ...
    function Delete_prop_NotificationTemplate(id) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: `{{ url('management/notificationtemplate/delete') }}`,
                data: {
                    'id': id
                },
                success: function(response) {
    ...
    $('#NotificationTemplatefileUpload').change(function(event) {
        ...
        $.ajax({
            url: `{{ url('/management/NotificationTemplate-image') }}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                ...
                $('#prop_NotificationTemplateimage').val(response
                    .fileName); // Set file name in hidden field
                $('#image_preview').attr('src', '/' + 'NotificationTemplate_image/' + response.fileName)
    ...
    function deleteUploadedImage() {
        var fileName = $('#prop_NotificationTemplateimage').val();
        if (!fileName) {
        ...
        $.ajax({
            url: `{{ url('/management/delete-NotificationTemplate-image') }}`,
            type: 'POST',
            data: {
                file: fileName
            },
            success: function(response) {
                ...
                $('#image_preview').attr('src', '').hide();
                $('#delete_image_btn').hide();
                $('#prop_NotificationTemplateimage').val('');
            },
            error: function(xhr, status, error) {
