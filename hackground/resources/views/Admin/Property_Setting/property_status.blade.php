...
<div class="app-main__inner">
    ...
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>Property Status
                    <div class="page-title-subheading">Property Setting &gt; Property Status List</div>
                </div>
            </div>
    ...
    <div class="main-card mb-3 card">
        ...
        <h4>Property Status List</h4>

                <div class="btn-actions-pane-right">
                    <button type="button" class="btn btn-sm btn-primary" onclick="add_prop_status()">Add
                        Status</button>
                </div>
                ...
                <table id="status" class="mb-0 table">
                    ...
                    <tbody id="user">
                        ...
                        <tr>
                            ...
                            <td>{{ $item->order }}</td>
                            <td>
                                <img src="{{ asset('user_upload/status/' . $item->icon) }}" alt="N/A"
...
<script>
            ...
            $.get(`{{ url('/property/status-details') }}/${id}`, function(data) {
                ...
                data.forEach(function(status) {
                    ...
                    if (status.lang === 'en') {
                        ...
                        var imageSrc = `{{ asset('user_upload/status') }}/${status.icon}`;
                        if (status.icon) {
                            $('#image_preview').attr('src', imageSrc).show();
                            $('#delete_image_btn').show();
                        }
                        $('#filename').val(status.icon);
    ...
    $('#fileUpload').change(function(event) {
        ...
        $.ajax({
            url: `{{ url('property/status/upload') }}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    function deleteUploadedImage() {
        ...
        $.ajax({
            url: `{{ url('property/status/image-delete') }}`,
            type: 'POST',
            data: {
                file: fileName
            },
