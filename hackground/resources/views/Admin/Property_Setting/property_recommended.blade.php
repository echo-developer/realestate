...
<div class="app-main__inner">
            ...
            <div class="row justify-content-end">
                <div class="col-xl-4 col-lg-6">
                    <div class="input-group">
                        <input class="form-control" id="prop_recommended_search" placeholder="Search..." name="term"
                            value="{{ request('term') }}" />
    ...
    <div class="main-card mb-3 card">
        ...
        <h4>Property Recommended List</h4>
            <div class="btn-actions-pane-right">
                <button type="button" class="btn btn-sm btn-primary" onclick="add_prop_recommended()">Add Recommended</button>
            </div>
        </div>
        <div class="card-body">
                ...
                <table class="mb-0 table">
                    ...
                    <tbody id="user">
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <a href="javascript:void(0)" onclick="Edit_prop_recommended('{{ $item->id }}')" class="me-2"><i class="bi bi-pencil-square text-success fa-md "></i></a>
                                <a href="javascript:void(0)" onclick="Delete_prop_recommended('{{ $item->id }}')"><i class="bi bi-trash3-fill text-danger fa-md"></i></a>
                            </td>
                        </tr>
...
@endsection
@section('modals')
<div class="modal fade" id="prop_recommended" tabindex="-1" role="dialog"
    aria-labelledby="prop_recommendedaddEditModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">

                <h5 class="modal-title" id="prop_recommendedAddEditModalLabel"></h5>
            ...
            <div class="modal-footer">
                <button type="button" onclick="add_edit_prop_recommended()" id="prop_recommendedButton"
...
<script>
    function add_prop_recommended() {
        ...
        prop_recommendedAddEdit('Property Recommended Add', 'Add');
    }

    function Edit_prop_recommended(id) {
        console.log(id);
        $('.form-control').removeClass('is-invalid');
        $('.invalid-feedback').empty();
        prop_recommendedAddEdit('Property Recommended Edit', 'Update', id);
    }

    function prop_recommendedAddEdit(title, buttonText, id = null) {
        $('#prop_recommendedAddEditModalLabel').text(title);
        $('#prop_recommendedButton').text(buttonText);
        $('#prop_recommendedformData')[0].reset();
        if (id) {
            $.get(`{{ url('/property/recommended-details') }}/${id}`, function(data) {
                $('#prop_recommendedId').val(data[0].recommended_id);
                data.forEach(function(recommended) {
                    $('#name_' + recommended.lang).val(recommended.name);
                    if (recommended.lang === 'en') {
                        $('#order').val(recommended.order);
                        $('input[name="status"][value="' + recommended.status + '"]').prop(
                            'checked', true);
                    }
                });
            });
        }
        $('#prop_recommended').modal('show');
    }

    function add_edit_prop_recommended() {
        var data = $("#prop_recommendedformData").serializeArray();
        $.ajaxSetup({
            headers: {
        ...
        var url = $('#prop_recommendedId').val() ?
            `{{ url('/property/edit-property-recommended') }}` :
            `{{ url('/property/add-property-recommended') }}`;
    ...
    $('.recommended_prop_status').change(function() {
        ...
        $.ajax({
            type: 'POST',
            url: `{{ url('property/recommended_status') }}`,
            data: {
                'status': status,
                'id': id
            },
    ...
    function Delete_prop_recommended(id) {
        ...
        if (result) {
            ...
            $.ajax({
                type: 'POST',
                url: `{{ url('property/recommended-delete') }}`,
                data: {
                    'id': id
                },
                success: function(response) {
