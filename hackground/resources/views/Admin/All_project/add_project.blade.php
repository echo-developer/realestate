          ...
          <div id="step-3" style="display:none;">
            <div class="row gx-3">
              <div class="col-lg-6 col-12">
                <div class="form-floating mb-3">
                  <select class="selectpicker hide-tick" name="city" id='city' data-width="fit" data-size="7" title="Choose City">
              ...
              <div class="col-lg-6 col-12">
                <div class="form-floating mb-3">
                  <select class="selectpicker hide-tick" id="locality" name="locality" data-width="fit" data-size="7" title="Choose Locality">
...
<script>
  $(document).ready(function() {
    ...
    function fetchLocalities(cityId, preselected = null) {
      $localitySelect.empty(); // Clear options first

      if (!cityId) return;

      $.ajax({
        url: `{{ url('get-locality') }}`,
        type: 'GET',
        data: {
          city_id: cityId
        },
        success: function(response) {
          if (response.localities && response.localities.length > 0) {
            ...
            $.each(response.localities, function(index, locality) {
              const selected = locality.id == preselected ? 'selected' : '';
              $localitySelect.append(
                `<option value="${locality.id}" ${selected}>${locality.name}</option>`
              );
            });
          } else {
            // Show only when user opens dropdown
            $localitySelect.append(
              '<option value="" disabled>No localities available</option>'
            );
          }

          $localitySelect.selectpicker('refresh');
        },
        error: function(xhr) {
          console.error('Error loading localities', xhr);
          $localitySelect
            .empty()
            .append('<option value="" disabled>Error loading localities</option>');
          $localitySelect.selectpicker('refresh');
        }
      });
    }
