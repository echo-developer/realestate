            ...
            if (!empty($tower_data)) {
                foreach ($tower_data as $items) {
                    ...
                    foreach ($items['floor_data'] as $floor) {
                        ...
                        Log::info('Floor ID:', ['floor_id' => $floorModel->id]);
                        foreach ($floor['bhk_configurations'] as $bhkdata) {
                            ...
                            $prop_ID = !empty($bhkdata['property_id'])
                                ...
                                : PrefProperty::create([
                                    'uid' => $user_id,
                                    'name' => get_project_property_name($bhk_type, $items['projectName']),
        ...
        try {
            ...
            foreach ($projectProperties as $tower) {
                ...
                foreach ($tower->floors as $floor) {
                    ...
                    $floorData[] = [
                        'flat_no' => $floor->id, // Assuming `flat_no` is stored as `id` or another field
                        'floor_no' => $floor->floor_no,
