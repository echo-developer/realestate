...
class CustomLandmarksAddController extends Controller
{
    public function insertLandmarks()
    {
        ...
        foreach ($pid as $index => $propertyId) {
            $localityId = $locality[$index] ?? null;

            if ($localityId === null) {
                continue; 
            }
            $this->savePropertyLandmarks($propertyId, $localityId);
        }

        return response()->json(['status' => 1, 'message' => 'Bulk insertion completed.']);
    }

    private function savePropertyLandmarks($propertyId, $localityId)
    {
        $radius = 2;
        ...
        $property_uid = DB::table('properties')
            ->where('id', $propertyId)
            ->pluck('uid');
