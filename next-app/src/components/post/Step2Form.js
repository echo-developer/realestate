        ...
        <>
            ...
            {propertyTypeData.map((category) => (
              <React.Fragment key={category.category_id}>
                <input
                  ...
                  onClick={() => handleSaveCategoryId(category.category_id)}
                />
                <label
          ...
          {propertyForData.length > 0 && (
            <>
              ...
              <div
                className={`btn-group btn-group-light d-flex mb-3 ${
                  errors.property_for ? "validation-error" : ""
                }`}
                role="group"
              >
                {propertyForData.map((subcategory) => (
                  <React.Fragment key={subcategory.sub_category_id}>
                    <input
                      ...
                      checked={
                        formData.property_for === subcategory.sub_category_id
                      }
                      onChange={handleChange}
                      value={subcategory.sub_category_id}
                      onClick={() =>
                        handleSelectPropertyFor(subcategory?.subcategory_key)
                      }
                    />
                    <label
                      className="btn btn-outline-light"
              ...
              {errors.property_for && (
                <div className="error-text">{errors.property_for}</div>
              )}
            </>
          )}
