...
const PropertyCompletionStatus = ({ propertyData }) => {
    ...
    const completionData = fields.map(({ key, label, weight }) => ({
        label,
        status: propertyData[key] ? "✔️ Completed" : "❌ Pending",
        weight: propertyData[key] ? weight : 0
    }));

    const completionPercentage = completionData.reduce((acc, field) => acc + field.weight, 0);
    ...
    const doughnutData = {
        ...
        datasets: [
            {
                data: [
                    completionData.reduce((acc, item) => acc + (item.weight ? item.weight : 0), 0),
                ...
                <ListGroup>
                    {completionData.map((item, index) => (
                        <ListGroup.Item key={index} className="d-flex justify-content-between">
                            ...
                            <span className={item.status.includes("Pending") ? "text-danger" : "text-success"}>
                                {item.status}
                            </span>
                        </ListGroup.Item>
                    ))}
